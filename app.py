...
# Car data for brands, models, fuel types, and seats
car_data = {
...
def predictdata():
    brands = car_data.keys()
    return render_template('form.html', brands=brands)
    logging.info(brands)

@app.route('/get_models', methods=['POST'])
def get_models():
    brand = request.form['brand']
    models = car_data.get(brand, [])
    logging.info(models)
    return jsonify(models)

@app.route('/get_car_info', methods=['POST'])
def get_car_info():
    car_name = request.form['car_name']
    info = car_info.get(car_name, ([], []))
    logging.info(info)
    return jsonify(info)
...
def predict():
    if request.method=='POST':
        ...
        logging.info(f'predictions is done {result}')
        return render_template('form.html',car_prices=result)
        logging.info('Whole process is complete ')
